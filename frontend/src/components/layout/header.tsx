...
import { useMobileMenu } from "../providers/client-provider";
...
export function Header() {
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Redirect to home page when user starts typing
    if (value.length > 0 && pathname !== "/") {
      router.push("/");
    }
  };
  return (
    ...
    //             className="w-full pl-12 pr-20 py-3 rounded-xl border border-border/60 bg-card/50 backdrop-blur-sm text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all duration-200 hover:bg-card/70 focus:bg-card shadow-sm"
    //           />
    ...
    //                 </div>
    //               ) : (
    //                 <div className="text-xs text-gray-400">Min 2 chars</div>
    //               )}
    //             </div>
    //           )}
    //         </div>
    ...
    //           className="h-9 w-9 rounded-xl bg-black dark:bg-white active:scale-95 transition-all duration-200 text-white dark:text-black shadow-sm"
    //         >
    ...
    //           value={searchQuery}
    //           onChange={handleSearchChange} // Updated handler
    //           className="w-full pl-12 pr-8 py-2.5 rounded-xl border border-border/60 bg-card/50 backdrop-blur-sm text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all duration-200"
    //         />
    ...
    <header className="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl bg-background/80 border-b border-border/40 supports-[backdrop-filter]:bg-background/60">
      ...
      {/* Mobile Menu Dropdown - Inside header container */}
      {mobileMenuOpen && (
        <div className="md:hidden px-4 py-4 space-y-3 bg-background/95 border-t border-border/40 backdrop-blur-lg">
          {/* Mobile Search */}
          <div className="relative w-full group">
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
              {isSearching ? (
